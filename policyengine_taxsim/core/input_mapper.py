...
def add_additional_units(state, year, situation, taxsim_vars):
    ...
    # TAXSIM's Maryland siitax is state-only, so zero PE's county tax (and with
    # it the local credits capped at it). Mirrors
    # PolicyEngineRunner._build_configured_sim so both execution paths agree;
    # see tests/test_md_local_tax_parity.py.
    if state.lower() == "md":
        tax_unit["md_local_income_tax_before_credits"] = {str(year): 0}
