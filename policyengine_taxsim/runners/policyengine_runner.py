...
try:
    from ..core.utils import load_variable_mappings, SOI_TO_FIPS_MAP, get_state_code, get_state_number, to_roundedup_number
    from ..core.input_mapper import set_taxsim_defaults
except ImportError:
    from policyengine_taxsim.core.utils import load_variable_mappings, SOI_TO_FIPS_MAP, get_state_code, get_state_number, to_roundedup_number
...
class TaxsimMicrosimDataset(Dataset):
    ...
    def generate(self) -> None:
        ...
        for year in unique_years:
            ...
            for household_idx, (_, row) in enumerate(year_data.iterrows()):
                ...
                for person_idx in range(n_people):
                    ...
                    if person_idx == 0:  # Primary taxpayer
                        ...
                        spouse_age = int(row['sage']) if row['sage'] > 0 else int(row['page'])
                        ...
                        dep_age = int(row.get(age_col, 10)) if row.get(age_col, 0) > 0 else 10
            ...
            data["co_child_care_subsidies"][year] = np.zeros(n_year_records)  # Prevent Colorado subsidy calculations
            ...
            data["state_fips"][year] = np.array([SOI_TO_FIPS_MAP.get(int(s), 6) for s in year_data['state'].values])
...
class PolicyEngineRunner(BaseTaxRunner):
    ...
    def run(self, show_progress: bool = True) -> pd.DataFrame:
        ...
        """
        if show_progress:
            print(f"Running PolicyEngine Microsimulation on {len(self.input_df)} records")
        ...
        try:
            ...
            years = sorted(set(self.input_df['year'].unique()))
            problematic_vars = [
                'ca_child_care_subsidies', 'ne_child_care_subsidies'  # State-specific subsidies (co_child_care_subsidies handled in dataset)
            ]
            ...
            try:
                ...
                for idx, (_, row) in enumerate(year_data.iterrows()):
                    ...
                    for taxsim_var, mapping in pe_to_taxsim.items():
                        ...
                        if taxsim_var in ["taxsimid", "year", "state", "fiitax", "siitax"]:
                        ...
                        if not pe_var or pe_var in ["taxsimid", "get_year", "get_state_code", "na_pe"]:
                            continue
