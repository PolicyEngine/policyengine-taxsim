...
class TaxsimRunner(BaseTaxRunner):
    ...
    REQUIRED_COLUMNS = [
        "year", "state", "mstat", "pwages", "depx", 
        "mortgage", "taxsimid", "idtl"
    ]
    
    ADDITIONAL_COLUMNS = [
        "page", "sage", "swages", "psemp", "ssemp", "dividends",
        "intrec", "stcg", "ltcg", "otherprop", "nonprop", "pensions",
        "gssi", "pui", "sui", "transfers", "rentpaid", "proptax",
        "otheritem", "childcare", "scorp"
    ]
    ...
    def _validate_executable(self):
        ...
        if not self.taxsim_path.exists():
            raise FileNotFoundError(f"TAXSIM executable not found at: {self.taxsim_path}")
        ...
        try:
            ...
            if os.path.exists(output_file):
                with open(output_file, 'r') as f:
                    raw_output = f.read()
                raise Exception(f"Failed to parse TAXSIM output: {e}\nRaw output:\n{raw_output}")
            else:
