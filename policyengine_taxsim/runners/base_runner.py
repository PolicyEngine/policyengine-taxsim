...
class BaseTaxRunner(ABC):
    ...
    def sample(self, n: int, random_state: int = 42) -> "BaseTaxRunner":
        ...
        if n >= len(self.input_df):
            print(
                f"Warning: Sample size ({n}) >= data size ({len(self.input_df)}). Returning original data."
            )
    ...
    def describe_data(self) -> dict:
        ...
        description = {
            ...
            "memory_usage_mb": self.input_df.memory_usage(deep=True).sum()
            / 1024
            / 1024,
        }
...
class ProgressTracker:
    ...
    def update(self, current: int):
        ...
        print(
            f"{self.description}: {current}/{self.total} ({percentage:.1f}%)", end="\r"
        )
