...
class ComparisonStatistics:
    ...
    def detailed_report(self) -> str:
        ...
        report.append("-" * 35)
        report.append(
            f"  Matches:     {stats['federal_tax']['matches']:,} ({stats['federal_tax']['match_percentage']:.2f}%)"
        )
        ...
        report.append("-" * 32)
        report.append(
            f"  Matches:     {stats['state_tax']['matches']:,} ({stats['state_tax']['match_percentage']:.2f}%)"
        )
        ...
        if "state" in self.input_data.columns:
            ...
            for state_fips, total_households in state_counts.items():
                ...
                breakdown[state_code] = {
                    ...
                    "federal_match_rate": (
                        (
                            (total_households - federal_mismatches)
                            / total_households
                            * 100
                        )
                    ...
                    "state_match_rate": (
                        ((total_households - state_mismatches) / total_households * 100)
                        if total_households > 0
                        else 0
                    ),
                }
    ...
    def print_summary(self):
        ...
        print(
            f"Federal Tax Matches: {stats['federal_tax']['matches']:,} ({stats['federal_tax']['match_percentage']:.2f}%)"
        )
        print(
            f"State Tax Matches: {stats['state_tax']['matches']:,} ({stats['state_tax']['match_percentage']:.2f}%)"
        )
