...
class TaxComparator:
    ...
    def _prepare_data(self):
        ...
        self.policyengine_results.columns = self.policyengine_results.columns.str.lower()
    ...
    def _compare_tax_column(self, 
                           col_name: str, 
                           tolerance: float, 
                           tax_type: str) -> Tuple[np.ndarray, List[MismatchRecord]]:
        """Compare a specific tax column between datasets"""
        mismatches = []
        
        if (col_name not in self.taxsim_results.columns or 
            col_name not in self.policyengine_results.columns):
    ...
    def compare(self) -> 'ComparisonResults':
        ...
        federal_matches, federal_mismatches = self._compare_tax_column(
            self.config.federal_tax_col,
            self.config.federal_tolerance,
            "federal"
        )
        
        # Compare state taxes
        state_matches, state_mismatches = self._compare_tax_column(
            self.config.state_tax_col,
            self.config.state_tolerance,
            "state"
        )
...
class ComparisonResults:
    ...
    def save_mismatches(self, 
        ...
        if self.federal_mismatches:
            ...
            print(f"Saved {len(self.federal_mismatches)} federal tax mismatches to: {federal_path}")
        ...
        if self.state_mismatches:
            ...
            print(f"Saved {len(self.state_mismatches)} state tax mismatches to: {state_path}")
    
    def _create_mismatch_dataframe(self, 
                                  mismatches: List[MismatchRecord], 
                                  input_data: pd.DataFrame = None) -> pd.DataFrame:
        ...
        if input_data is not None:
            ...
            if tax_type == "federal":
                result_df['taxsim_federal_tax'] = result_df['taxsimid'].map(lambda x: mismatch_map[x].taxsim_value)
                result_df['policyengine_federal_tax'] = result_df['taxsimid'].map(lambda x: mismatch_map[x].policyengine_value)
                result_df['federal_difference'] = result_df['taxsimid'].map(lambda x: mismatch_map[x].difference)
            else:  # state
                result_df['taxsim_state_tax'] = result_df['taxsimid'].map(lambda x: mismatch_map[x].taxsim_value)
                result_df['policyengine_state_tax'] = result_df['taxsimid'].map(lambda x: mismatch_map[x].policyengine_value)
                result_df['state_difference'] = result_df['taxsimid'].map(lambda x: mismatch_map[x].difference)
