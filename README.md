...
PolicyEngine maintains and develops policyengine-taxsim. The repository is public and open source, and releases keep the model current as federal and state tax law changes.
...
When PolicyEngine migrates its rules engine to the [Axiom](https://axiom.org) backend, the TAXSIM emulator moves with it. The surfaces above stay stable, and we will make the transition as invisible as possible. Using PolicyEngine directly adds capabilities beyond the TAXSIM format (benefit programs, and more accurate tax calculations from variables TAXSIM doesn't support). Axiom will add substantially more: it follows the law exactly.
