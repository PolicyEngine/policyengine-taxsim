...
def h5_to_dataframe(h5_file_path, person_fields=None, household_fields=None, tax_unit_fields=None, spm_unit_fields=None, year="2024", sample_size=0):
    """
    ...
    if person_fields is None:
        person_fields = [
            ...
            "social_security_retirement", "person_tax_unit_id",
            "person_household_id", "person_spm_unit_id", 
            "is_household_head", "is_female", "rent",
            "partnership_s_corp_income"
        ]
    
    if household_fields is None:
        household_fields = [
            "household_id", "state_fips"
        ]
    ...
    if spm_unit_fields is None:
        spm_unit_fields = [
            "spm_unit_id", "spm_unit_pre_subsidy_childcare_expenses"
        ]
    ...
    with h5py.File(h5_file_path, 'r') as f:
        ...
        if household_data:
            ...
            household_index = np.arange(len(household_data["household_id"]) if "household_id" in household_data else 0)
            household_df = pd.DataFrame(household_data, index=household_index)
            
            # Merge household data to person data
            if "person_household_id" in person_df.columns and "household_id" in household_df.columns:
                # Create a mapping from household_id to household data
                household_mapping = {
                    hid: {"state_fips": sf} 
                    for hid, sf in zip(
                        household_df["household_id"],
                        household_df["state_fips"] if "state_fips" in household_df else [0] * len(household_df)
                    )
                }
                
                # Apply the mapping to add household data to person records
                if "state_fips" not in person_df.columns and "state_fips" in household_fields:
            ...
            for field in ["deductible_mortgage_interest", "real_estate_taxes"]:
                if field in f and f"{year}" in f[field]:
                    ...
                    if len(field_values) == len(tax_unit_data["tax_unit_id"]):
                        ...
                        field_mapping = {tuid: val for tuid, val in zip(tax_unit_data["tax_unit_id"], field_values)}
        ...
        if spm_unit_data and "spm_unit_id" in spm_unit_data:
            ...
            if "spm_unit_pre_subsidy_childcare_expenses" in spm_unit_data:
                ...
                if "person_spm_unit_id" in person_df.columns:
                    person_df["childcare_expenses"] = person_df["person_spm_unit_id"].apply(
                        lambda spm_id: childcare_mapping.get(spm_id, 0)
                    )
...
def h5_to_taxsim_format(h5_file_path, year, output_csv_path=None, sample_size=0):
    ...
    person_fields = [
        ...
        "taxable_pension_income", "unemployment_compensation",
        "social_security_retirement", "person_tax_unit_id",
        "person_household_id", "person_spm_unit_id", "is_household_head", 
        "is_female", "rent", "partnership_s_corp_income"
    ]
    ...
    for i, (tax_unit_id, group) in enumerate(tax_units, 1):
        ...
        if is_head.any():
            ...
            dependents = group[~is_head if spouse is None else ~(is_head | (group.index == spouse.name))]
            ...
            taxsim_record = {
                ...
                # Income - spouse
                "swages": float(spouse.get("employment_income", 0)) if spouse is not None else 0,
                "ssemp": float(spouse.get("self_employment_income", 0)) if spouse is not None else 0,
                ...
                "sui": float(spouse.get("unemployment_compensation", 0)) if spouse is not None else 0,
                
                # Other income and deductions
                "transfers": 0,
                "rentpaid": float(group["rent"].sum()) if "rent" in group.columns else 0,
                ...
                "scorp": float(group["partnership_s_corp_income"].sum()) if "partnership_s_corp_income" in group.columns else 0,
...
def run_taxsim(input_csv_path, output_csv_path):
    ...
    required_columns = ["year", "state", "mstat", "pwages", "depx", "mortgage", "taxsimid", "idtl"]
    
    # Additional columns we'll use
    additional_columns = [
        "page", "sage", "swages", "psemp", "ssemp", "dividends", "intrec", "stcg", 
        "ltcg", "otherprop", "nonprop", "pensions", "gssi", "pui", "sui", 
        "transfers", "rentpaid", "proptax", "otheritem", "childcare", "scorp"
    ]
    ...
    # Run TAXSIM
    if system != "windows":
        cmd = f'cat "{formatted_input_path}" | "{str(taxsim_path)}" > "{raw_output_path}"'
    else:
...
def compare_results(taxsim_output, pe_output, taxsim_input=None, output_dir=None, year=None):
    """
    ...
    if federal_tax_col in taxsim_output.columns and federal_tax_col in pe_output.columns:
        ...
        for i, matches in enumerate(federal_matches):
            if not matches:
                record = {
                    ...
                    "difference": taxsim_output.iloc[i][federal_tax_col] - pe_output.iloc[i][federal_tax_col]
                }
        ...
        for i, matches in enumerate(state_matches):
            if not matches:
                record = {
                    ...
                    "difference": taxsim_output.iloc[i][state_tax_col] - pe_output.iloc[i][state_tax_col]
                }
    ...
    if len(federal_mismatches) > 0 or len(state_mismatches) > 0:
        print(f"Found {len(federal_mismatches)} federal tax mismatches and {len(state_mismatches)} state tax mismatches")
        
        # Create DataFrames for mismatches
        federal_mismatch_df = pd.DataFrame(federal_mismatches) if federal_mismatches else pd.DataFrame()
        state_mismatch_df = pd.DataFrame(state_mismatches) if state_mismatches else pd.DataFrame()
        ...
        if taxsim_input is not None and output_dir is not None:
            ...
            if len(federal_mismatches) > 0:
                ...
                federal_full = taxsim_input[taxsim_input["taxsimid"].isin(federal_ids)].copy()
                
                # Add TAXSIM and PolicyEngine results
                for idx, row in federal_full.iterrows():
                    taxsim_id = row["taxsimid"]
                    mismatch = federal_mismatch_df[federal_mismatch_df["taxsimid"] == taxsim_id]
                    
                    if not mismatch.empty:
                        federal_full.loc[idx, "taxsim_federal"] = mismatch.iloc[0]["taxsim_federal"]
                        federal_full.loc[idx, "pe_federal"] = mismatch.iloc[0]["pe_federal"]
                        federal_full.loc[idx, "federal_difference"] = mismatch.iloc[0]["difference"]
                
                # Save to CSV
                year_suffix = f"_{year}" if year else ""
                federal_output_path = output_dir / f"federal_mismatches{year_suffix}.csv"
            ...
            if len(state_mismatches) > 0:
                ...
                state_full = taxsim_input[taxsim_input["taxsimid"].isin(state_ids)].copy()
                
                # Add TAXSIM and PolicyEngine results
                for idx, row in state_full.iterrows():
                    taxsim_id = row["taxsimid"]
                    mismatch = state_mismatch_df[state_mismatch_df["taxsimid"] == taxsim_id]
                    
                    if not mismatch.empty:
                        state_full.loc[idx, "taxsim_state"] = mismatch.iloc[0]["taxsim_state"]
                        state_full.loc[idx, "pe_state"] = mismatch.iloc[0]["pe_state"]
                        state_full.loc[idx, "state_difference"] = mismatch.iloc[0]["difference"]
...
def process_cps_h5(file_path, year, output_dir=None, sample_size=0, save_csv=False, use_h5=False):
    """
    ...
    if use_h5:
        ...
        if sample_size > 0 and len(taxsim_df) > sample_size:
            ...
            if output_dir:
                sample_path = output_dir / f"sampled_{os.path.basename(taxsim_input_path)}"
    ...
    try:
        ...
        print(f"Federal Tax Matches: {comparison['federal_match_count']} ({comparison['federal_match_pct']:.2f}%)")
        print(f"State Tax Matches: {comparison['state_match_count']} ({comparison['state_match_pct']:.2f}%)")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Process CPS H5 file or CSV with TAXSIM and PolicyEngine")
    parser.add_argument("file", help="Path to the H5 file or CSV file")
    parser.add_argument("--use-h5", action="store_true", help="Flag indicating the input is an H5 file")
    parser.add_argument("--extract-only", action="store_true", help="Extract data from H5 file to CSV and exit (no comparison)")
    ...
    if args.extract_only:
        ...
        if args.sample > 0 and len(taxsim_df) > args.sample:
            ...
            output_csv_path = output_dir / f"taxsim_input_{year_str}_sample_{args.sample}.csv"
        
        # Save to CSV
        taxsim_df.to_csv(output_csv_path, index=False)
        print(f"Successfully extracted {len(taxsim_df)} tax units to: {output_csv_path}")
        print("Note: This CSV is ready for TAXSIM processing and can be used directly with this script.")
        sys.exit(0)
    
    # Process the file (comparison mode)
    process_cps_h5(args.file, args.year, args.output_dir, args.sample, args.save_csv, args.use_h5)